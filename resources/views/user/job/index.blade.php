...
@section('content')
                        ...
                        <ul class="dropdown-menu overflow-y-auto" style="height: 250px" aria-labelledby="filterDropdown">
                            <li>
                                <a class="dropdown-item filter-all" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'all')">All</a>
                            </li>
                            <li>
                                <a class="dropdown-item filter-saved" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'saved')">Saved</a>
                            </li>
                            <li>
                                <a class="dropdown-item filter-applied" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'applied')">Applied</a>
                            </li>
                            {{-- <li>
                                <a class="dropdown-item filter-active" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'active')">Active</a>
                            </li>
                            <li>
                                <a class="dropdown-item filter-featured" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'featured')">Featured</a>
                            </li>
                            <li>
                                <a class="dropdown-item filter-verified" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'verified')">Verified</a>
                            </li> --}}
                            <li>
                                <a class="dropdown-item filter-newest" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'newest')">Newest</a>
                            </li>
                            <li>
                                <a class="dropdown-item  filter-oldest" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'oldest')">Oldest</a>
                            </li>
                            <li>
                                <a class="dropdown-item filter-highest" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'salary_high')">
                                    Highest Salary
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item filter-lowest" href="#"
                                    onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', 'salary_low')">
                                    Lowest Salary
                                </a>
                            </li>
                            {{-- work_types --}}
                            @foreach ($work_types as $value)
                                <li>
                                    <a class="dropdown-item filter-work-type-{{ Str::lower($value) }}" href="#"
                                        onclick="fetchJobs('{{ route('user.job.getAllJobs') }}', '{{ Str::lower($value) }}')">
                                        {{ $value }}
                                    </a>
                                </li>
                            @endforeach
    ...
    <script>
        const fetchJobs = (
            url = "{{ route('user.job.getAllJobs') . '?page=1' }}",
            filter = 'all',
        ) => {
                        ...
                        response.data.map(function(job) {
                            $('#jobs').append(`
                                <tr>
                                    ...
                                    <td>
                                        ${job.is_applied ? '<a href="' + "{{ route('user.job.unapply', '') }}/" + job.id + '" class="badge bg-success">Applied</a>' : '<a href="' + "{{ route('user.job.apply', '') }}/" + job.id + '" class="badge bg-info">Apply Now</a>'}
                                    </td>
                                    <td>
                                        ${job.is_saved ? '<a href="' + "{{ route('user.job.unsaveJob', '') }}/" + job.id + '" class="badge bg-success">Saved</a>' : '<a href="' + "{{ route('user.job.saveJob', '') }}/" + job.id + '" class="badge bg-info">Save Now</a>'}
                                    </td>
                                </tr>
                            `);
                        });
